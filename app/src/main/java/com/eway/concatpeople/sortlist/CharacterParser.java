...
public class CharacterParser {
	...
	private String resource; // 数据

	private static CharacterParser characterParser = new CharacterParser();  // 使用静态方法初始化对象
	...
	/**
	 * * 单字解析
	 * * @param str  单个的汉字,其中拼音很定的字母的ASCII的个数大于2
	 * * @return  单个字符的拼音
	 * */
	public String convert(String str) {
		String result = null;
		int ascii = getChsAscii(str);      //调用汉字转ASCII码的方法，转化单个汉字的ASCII值
		if (ascii > 0 && ascii < 160) {    //GB2312中，0-160 不是拼音,则直接返回string类型的数据
			result = String.valueOf((char) ascii);
		} else {                                   // 160开始是汉字拼音
			for (int i = (pyvalue.length - 1); i >= 0; i--) {
				if (pyvalue[i] <= ascii) {   // 倒循环，查找acill值相同的拼音
					result = pystr[i];       // 取出这个ascii对应的的 拼音
					break;
				}
			}
		}
	...
	public String getSelling(String chs) {
		...
		for (int i = 0; i < chs.length(); i++) {  // 循环的解析词组每个汉字的拼音
			key = chs.substring(i, i + 1);        // 用key存放取出的字符串汉字的第i个字
			if (key.getBytes().length >= 2) {       //使用getBytes()获取其拼音的字母的ascii值，如果字母个数>2，说明是拼音
				value = (String) convert(key);    // 调用单词拼音的方法，转换其值
				if (value == null) {
					value = "unknown";
				}
			} else {           // 如果获取的汉字的拼音只有一个字母，如a,则直接获取这个值
				value = key;         //
			}
			buffer.append(value);
		}
